    ...
    IVersioned
{
    ...
    function rebalanceReserve() external whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
    ...
    /// @notice This function can be called by the owner only.
    function collectYield() external whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        _collectYield(true);
    }
    ...
    /// @notice This function can be called by the owner only.
    function setYieldRecipient(address yieldRecipient_) external whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        VaultBridgeTokenStorage storage $ = _getVaultBridgeTokenStorage();
    ...
    /// @notice This function can be called by the owner only.
    function setNativeConverters(NativeConverterInfo[] calldata nativeConverters_) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        VaultBridgeTokenStorage storage $ = _getVaultBridgeTokenStorage();
    ...
    /// @notice This function can be called by the owner only.
    function setMinimumDepositAmount(uint256 minimumYieldVaultDeposit_) external whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        VaultBridgeTokenStorage storage $ = _getVaultBridgeTokenStorage();
