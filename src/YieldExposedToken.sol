    ...
    IVersioned
{
    ...
    // @todo Consider removing the `whenNotPaused` modifier.
    ...
    /// @dev @note CAUTION! Do not forget to verify the `originAddress` and `originNetwork` - otherwise, an attacker could gain access unauthorized access to yeToken and other contracts.
    ...
    /// @param shares The required amount of yeToken to mint and lock up in LxLy Bridge. Assets from a dedicated migration fees fund may be used to offset transfer fees of the underlying token. If a migration cannot be completed due to insufficient assets, anyone can donate the underlying token to the migration fees fund. Please refer to `donateForCompletingMigration` for more information.
    ...
    function _completeMigration(uint32 originNetwork, uint256 shares, uint256 assets) internal {
        ...
        // A discrepancy is possible due to transfer fees of the underlying token. To offset the discrepancy, we mint more yeToken, backed by assets from the dedicated migration fees fund.
