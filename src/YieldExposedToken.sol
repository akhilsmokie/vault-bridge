    ...
    IVersioned
{
    ...
    /// @param minimumReservePercentage_ 100 is 100%.
    ...
    /// @param nativeConverter_ The address of Native Converter of this yeToken.
    ...
    function reservePercentage() external view returns (uint256) {
        ...
        return ($.reservedAssets * 100) / convertToAssets(totalSupply());
    }
    ...
    /// @dev CAUTION! The deployment procedure must ensure that contracts are deployed to the same addresses on Layer Ys and their ownership cannot be frontrun; otherwise, an attacker can gain access to yeToken on Layer X via `onMessageReceived`.
    ...
    /// @param shares The required amount of yeToken to mint and lock up in LxLy Bridge. Available yield may be used to offset transfer fees of the underlying token. If a migration cannot be completed (due to insufficient yield to cover for the discrepancy), anyone can `donate` to increase available yield.
    ...
    function _completeMigration(uint32 originNetworkId, uint256 shares, uint256 assets) internal {
        ...
        // @note Check rounding.
        uint256 assetsToReserve = (requiredAssets * $.minimumReservePercentage) / 100;
